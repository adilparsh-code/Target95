...
export async function generateMetadata({
  ...
  const title =
    BLOG_CATEGORY_LABELS[category as BlogCategory] || "Blog";
...
export default async function BlogCategoryPage({
  ...
  const title = BLOG_CATEGORY_LABELS[validCategory];
  ...
  try {
    ...
    console.error(
      `Failed to load blog articles for category: ${validCategory}`,
      error,
    );
    articles = [];
  }
    ...
    <main className="min-h-screen bg-slate-50 dark:bg-slate-950">
      ...
      <section className="border-b border-slate-200 bg-white dark:border-slate-800 dark:bg-slate-950">
        <Container className="py-14 sm:py-18">
          ...
          <p className="mt-8 text-sm font-bold uppercase tracking-[0.18em] text-blue-600 dark:text-blue-400">
            Target95+ Blog
          </p>
          ...
          <p className="mt-4 max-w-2xl text-slate-600 dark:text-slate-400">
            Fresh, useful content selected for students, teachers and
            learners.
          </p>
        </Container>
      </section>
          ...
          {articles.length === 0 ? (
            <div className="rounded-3xl border border-dashed border-slate-300 bg-white p-10 text-center dark:border-slate-700 dark:bg-slate-900">
              ...
              <p className="mx-auto mt-2 max-w-lg text-sm leading-6 text-slate-600 dark:text-slate-400">
                The automated editorial pipeline is collecting and preparing
                the first quality articles for this category.
              </p>
            </div>
          ) : (
            <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
              {articles.map((article) => (
                ...
                >
                  ...
                  <Link
                    href={`/blog/${validCategory}/${article.slug}`}
                    className="mt-5 inline-flex font-bold text-blue-600 hover:underline dark:text-blue-400"
                  >
                    Read article →
                  </Link>
                </article>
