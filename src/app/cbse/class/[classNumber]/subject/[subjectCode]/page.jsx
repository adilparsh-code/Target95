      ...
      <div className="mx-auto max-w-7xl">
        ...
        <div className="relative overflow-hidden rounded-3xl bg-gradient-to-r from-blue-600 via-indigo-600 to-purple-700 p-8 md:p-12 text-white shadow-2xl mb-8">
          ...
          <div className="relative z-10">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              ...
              <div className="bg-white/15 backdrop-blur-md rounded-2xl p-4 border border-white/20">
                <p className="text-3xl font-bold">{totalPracticalActivities}</p>
                <p className="text-blue-100 text-sm">Practical Activities</p>
              </div>
              <div className="bg-white/15 backdrop-blur-md rounded-2xl p-4 border border-white/20">
        ...
        <section className="mb-12">
          <div className="flex items-center justify-between mb-8">
            <h2 className="text-2xl md:text-3xl font-bold text-gray-900">Course Units</h2>
            <p className="text-gray-600 hidden md:block">Click on any unit to start learning</p>
          </div>
          ...
          <div className="grid gap-6 md:grid-cols-2">
            {units.map((unit, index) => (
              <Link
                ...
                className="group bg-white rounded-2xl border border-gray-200 p-6 shadow-sm hover:shadow-xl transition-all duration-300 transform hover:-translate-y-2 hover:border-blue-400 focus:outline-none focus:ring-4 focus:ring-blue-100"
              >
                <div className="flex items-start justify-between mb-4">
                  <div className="flex items-center gap-3">
                    <span className="flex items-center justify-center w-12 h-12 bg-gradient-to-br from-blue-500 to-indigo-600 text-white rounded-xl text-lg font-bold shadow-lg">
                      {unit.code}
                    </span>
                    <div>
                      <h3 className="text-xl font-bold text-gray-900 group-hover:text-blue-700 transition-colors">{unit.name}</h3>
                      <p className="text-sm text-gray-500">Unit {index + 1} of {units.length}</p>
                    </div>
                  </div>
                  <div className="flex gap-2">
                    {(unit.theory?.length > 0) && (
                    ...
                    {(unit.practicalActivities?.length > 0) && (
                      <span className="inline-flex items-center gap-1 bg-purple-50 text-purple-700 px-2.5 py-1 rounded-full text-xs font-semibold">
                        ...
                        {unit.practicalActivities.length}
                      </span>
                    )}
                  </div>
                </div>
                  ...
                  <div className="flex gap-4 text-sm text-gray-600">
                    {unit.theory?.length > 0 && (
                      <span className="flex items-center gap-1.5">
                        ...
                        {unit.theory.length} theory topics
                      </span>
                    )}
                    {unit.practicalActivities?.length > 0 && (
                      <span className="flex items-center gap-1.5">
                        <FlaskConical className="w-4 h-4 text-purple-500" />
                        {unit.practicalActivities.length} practicals
                      </span>
                    )}
                  </div>
        ...
        <section className="bg-white rounded-3xl border border-gray-200 p-8 shadow-sm mb-8">
          ...
          <div className="grid md:grid-cols-3 gap-6">
            <div className="bg-gradient-to-br from-amber-50 to-orange-50 rounded-2xl p-6 border border-amber-100 hover:shadow-lg transition-shadow">
              <Trophy className="w-10 h-10 text-amber-600 mb-4" />
              <h3 className="text-xl font-bold text-gray-900 mb-2">Mock Tests</h3>
              <p className="text-gray-600 mb-4">Test your knowledge with full-length mock exams designed according to CBSE pattern.</p>
              <Link href="/mock-test" className="inline-flex items-center gap-2 text-amber-700 font-semibold hover:underline">
                Take a test <ArrowLeft className="w-4 h-4 rotate-180" />
              </Link>
            </div>
            <div className="bg-gradient-to-br from-emerald-50 to-teal-50 rounded-2xl p-6 border border-emerald-100 hover:shadow-lg transition-shadow">
              ...
              <p className="text-gray-600 mb-4">Get personalized help with difficult concepts from our AI-powered tutor.</p>
            ...
            <div className="bg-gradient-to-br from-pink-50 to-rose-50 rounded-2xl p-6 border border-pink-100 hover:shadow-lg transition-shadow">
              <BookOpen className="w-10 h-10 text-pink-600 mb-4" />
              <h3 className="text-xl font-bold text-gray-900 mb-2">Study Notes</h3>
              <p className="text-gray-600 mb-4">Access comprehensive study notes and revision materials for all units.</p>
              <Link href="/study" className="inline-flex items-center gap-2 text-pink-700 font-semibold hover:underline">
                View notes <ArrowLeft className="w-4 h-4 rotate-180" />
              </Link>
            </div>
          </div>
        </section>
