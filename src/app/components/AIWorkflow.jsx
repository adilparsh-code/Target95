    ...
    <section className="relative overflow-hidden bg-slate-950 py-20 text-white sm:py-24 lg:py-28">
      ...
      <Container className="relative">
        <div className="mx-auto max-w-3xl text-center">
          <p className="type-label font-bold tracking-wide text-blue-300">AI-powered learning loop</p>
          <h2 className="mt-4 type-display font-black tracking-[-0.03em]">Your preparation gets smarter as you learn.</h2>
          <p className="mt-5 type-body-lg leading-7 text-slate-400">A simple loop: discover the gap, understand the concept, practise it and measure the improvement.</p>
        </div>
        ...
        <div className="relative mt-14 grid gap-4 md:grid-cols-2 lg:grid-cols-4">
          ...
          {steps.map((step) => { const Icon = step.icon; return <div key={step.number} className="relative rounded-2xl border border-white/10 bg-white/[0.055] p-6 backdrop-blur transition hover:border-blue-300/30 hover:bg-white/[0.08] sm:p-7"><div className="flex items-center justify-between"><div className="flex h-12 w-12 items-center justify-center rounded-2xl bg-blue-500/15 text-blue-200 ring-1 ring-blue-300/10"><Icon className="h-5 w-5" /></div><span className="type-label font-black tracking-wide text-slate-600">{step.number}</span></div><h3 className="mt-7 type-heading-sm font-extrabold">{step.title}</h3><p className="mt-3 type-body leading-6 text-slate-400">{step.description}</p><ArrowRight className="mt-7 h-4 w-4 text-slate-600" /></div>; })}
        </div>
      </Container>
    </section>
  );
