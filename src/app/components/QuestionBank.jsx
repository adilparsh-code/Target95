...
export default function QuestionBank() {
  ...
  return <div className="mx-auto w-full max-w-7xl px-4 py-8 sm:px-6 sm:py-10 lg:px-8">
    ...
    <section className="mt-6 rounded-[1.75rem] border border-slate-200 bg-white p-5 shadow-sm sm:p-7" aria-label="Question filters"><div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">
      <div className="relative md:col-span-2"><label htmlFor="question-search" className="sr-only">Search questions</label><span className="pointer-events-none absolute left-4 top-1/2 -translate-y-1/2 text-slate-500"><SearchIcon /></span><input id="question-search" value={filters.search} onChange={(event) => update("search", event.target.value)} onKeyDown={(event) => { if (event.key === "Enter") runSearch(); }} placeholder="Search questions, chapters, topics, or tags..." aria-label="Search questions" className={`${fieldClass} pl-11 pr-28`} /><button type="button" onClick={runSearch} className="absolute right-1.5 top-1/2 -translate-y-1/2 h-9 rounded-lg bg-blue-600 px-4 text-sm font-semibold text-white transition hover:bg-blue-700 focus:outline-none focus:ring-4 focus:ring-blue-100">Search</button></div>
