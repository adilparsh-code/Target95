...
export default function RedesignedHero({
  ...
  const availableSubjects = useMemo(() => {
    ...
    return subjectsData.filter((subject) => selectedClass.subjects.includes(subject.id) && !subject.comingSoon);
  }, [selectedClass]);
      ...
      <div className="relative mx-auto max-w-7xl px-5 py-14 sm:px-8 sm:py-18 lg:px-10 lg:py-24">
          ...
          <div className="grid items-center gap-12 lg:grid-cols-[1.05fr_.95fr]">
            <div>
              ...
              <h1 className="mt-6 text-4xl font-black tracking-tight text-slate-950 sm:text-5xl lg:text-7xl dark:text-white">
                Learn smarter.
                <span className="block bg-gradient-to-r from-blue-600 to-indigo-600 bg-clip-text text-transparent">Practice better.</span>
              ...
              <div className="mt-8 flex flex-col gap-3 sm:flex-row">
                ...
                <Link href="/mock-test" className="inline-flex min-h-12 items-center justify-center rounded-xl border border-slate-300 bg-white px-6 font-bold text-slate-800 transition hover:border-slate-400 hover:bg-slate-50 dark:border-slate-700 dark:bg-slate-900 dark:text-white dark:hover:bg-slate-800">Try a Mock Test</Link>
              </div>
              <div className="mt-8 grid max-w-xl grid-cols-3 gap-3 text-sm">
                {["Board-focused", "Chapter-wise", "AI-assisted"].map((item) => <div key={item} className="rounded-xl border border-slate-200 bg-white/80 p-3 text-center font-semibold text-slate-700 dark:border-slate-800 dark:bg-slate-900/80 dark:text-slate-200">{item}</div>)}
              </div>
            </div>

            <div id="boards" className="scroll-mt-24">
              <div className="mb-5">
                <p className="text-sm font-bold uppercase tracking-[0.18em] text-slate-500 dark:text-slate-400">Start here</p>
                <h2 className="mt-2 text-2xl font-black text-slate-950 sm:text-3xl dark:text-white">Choose your board</h2>
              </div>
              <div className="grid gap-5 sm:grid-cols-2">
                {Object.entries(boardClassSubjectMap).map(([id, item]) => (
                  <button key={id} type="button" onClick={() => onBoardSelect?.(id)} className="group rounded-3xl border border-slate-200 bg-white p-6 text-left shadow-sm transition hover:-translate-y-1 hover:border-slate-300 hover:shadow-xl focus:outline-none focus:ring-2 focus:ring-blue-500 dark:border-slate-700 dark:bg-slate-900 dark:hover:border-slate-600">
                    <div className={`flex h-14 w-14 items-center justify-center rounded-2xl bg-gradient-to-br ${item.accent} text-2xl text-white shadow-md`}>{id === "cisce" ? "🎓" : "🏫"}</div>
              ...
              <div>
                ...
                <div className="grid gap-4 sm:grid-cols-2">
                  {availableClasses.map((item) => (
                    <button key={item.id} type="button" onClick={() => onClassSelect?.(item)} className="rounded-2xl border border-slate-200 bg-white p-5 text-left shadow-sm transition hover:-translate-y-0.5 hover:shadow-lg focus:outline-none focus:ring-2 focus:ring-blue-500 dark:border-slate-700 dark:bg-slate-900">
              ...
              <div>
                ...
                <div className="grid gap-5 sm:grid-cols-2">
                  {availableSubjects.map((subject) => (
                    <Link key={subject.id} href={subject.href} className="group rounded-3xl border border-slate-200 bg-white p-6 shadow-sm transition hover:-translate-y-1 hover:shadow-xl dark:border-slate-700 dark:bg-slate-900">
        ...
        {!showClasses && !showSubjects && (
          <div className="mt-10 flex flex-wrap justify-center gap-3">
            <Link href="/question-bank" className="rounded-xl border border-slate-300 bg-white px-5 py-3 font-semibold text-slate-800 hover:bg-slate-50 dark:border-slate-700 dark:bg-slate-900 dark:text-white">Explore Question Bank</Link>
            <Link href="/ai-tutor" className="rounded-xl border border-slate-300 bg-white px-5 py-3 font-semibold text-slate-800 hover:bg-slate-50 dark:border-slate-700 dark:bg-slate-900 dark:text-white">Ask AI Tutor</Link>
          </div>
        )}
      </div>
    </section>
