    ...
    <div className="flex flex-col h-full bg-gray-50 dark:bg-gray-950 relative">
      <div className="bg-white dark:bg-gray-900 border-b border-gray-200 dark:border-gray-800 px-4 py-3 flex items-center justify-between gap-3">
        <div className="flex items-center min-w-0">
          <button onClick={onOpenSidebar} aria-label="Open chat history" className="lg:hidden p-2 mr-2 hover:bg-gray-100 dark:hover:bg-gray-800 rounded-lg">
        ...
        {!hasMessages ? (
          ...
          <div className="max-w-4xl mx-auto px-4 py-6">
            {messages.map((message, index) => (
              <MessageBubble key={index} message={message} onRegenerate={index === lastMessageIndex && message.role === "assistant" ? onRegenerate : null} isLastMessage={index === lastMessageIndex} />
            ))}
