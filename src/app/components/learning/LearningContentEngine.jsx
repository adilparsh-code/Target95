...
function SelectFilter({ label, value, options, onChange }) {
  return <label className="block text-xs font-semibold text-gray-700"><span className="mb-1 block">{label}</span><select value={value} onChange={(event) => onChange(event.target.value)} className="w-full rounded-lg border border-gray-300 bg-white px-3 py-2 text-sm text-gray-800"><option value="">All {label.toLowerCase()}s</option>{options.map((option) => <option key={option} value={option}>{option}</option>)}</select></label>;
}
...
export default function LearningContentEngine({ topic, allTopics }) {
  ...
  const completion = Math.round((attemptedInTopic / topic.questions.length) * 100);
  ...
  return <div className="space-y-6">
    ...
    <section id="learn" className="grid gap-4 md:grid-cols-2">{sections.map((section) => <div key={section} className="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm"><h3 className="font-bold text-gray-900">{section}</h3>{section === "Introduction" && <p className="mt-2 text-sm leading-6 text-gray-700">{topic.introduction}</p>}{section === "Definition" && <p className="mt-2 text-sm leading-6 text-gray-700">{topic.definition}</p>}{section === "Explanation" && <p className="mt-2 text-sm leading-6 text-gray-700">{topic.detailedExplanation}</p>}{section === "Real-life example" && <p className="mt-2 text-sm leading-6 text-gray-700">{topic.example}</p>}{section === "Syntax" && <pre className="mt-3 overflow-x-auto rounded-xl bg-slate-900 p-3 text-xs text-slate-100">{topic.syntax}</pre>}{section === "Important points" && <ul className="mt-2 list-disc space-y-1 pl-5 text-sm text-gray-700">{topic.importantPoints.map((item) => <li key={item}>{item}</li>)}</ul>}{section === "Key terms" && <div className="mt-3 flex flex-wrap gap-2">{topic.keyTerms.map((term) => <span key={term} className="rounded-full bg-blue-50 px-3 py-1 text-xs font-semibold text-blue-800">{term}</span>)}</div>}{section === "Common mistakes" && <ul className="mt-2 list-disc space-y-1 pl-5 text-sm text-gray-700">{topic.mistakes.map((item) => <li key={item}>{item}</li>)}</ul>}{section === "Tips & tricks" && <ul className="mt-2 list-disc space-y-1 pl-5 text-sm text-gray-700">{topic.tips.map((item) => <li key={item}>{item}</li>)}</ul>}{section === "Revision notes" && <ul className="mt-2 list-disc space-y-1 pl-5 text-sm text-gray-700">{topic.revisionNotes.map((item) => <li key={item}>{item}</li>)}</ul>}{section === "Summary" && <p className="mt-2 text-sm leading-6 text-gray-700">{topic.summary}</p>}{section === "FAQs" && <div className="mt-2 space-y-3">{topic.faqs.map((faq) => <div key={faq.question}><p className="text-sm font-semibold text-gray-800">{faq.question}</p><p className="text-sm text-gray-600">{faq.answer}</p></div>)}</div>}</div>)}</section>

    <section className="rounded-3xl border border-dashed border-blue-300 bg-blue-50 p-6"><h3 className="font-bold text-blue-950">Diagram / illustration</h3><p className="mt-1 text-sm text-blue-900">Reserved for a curriculum-reviewed diagram that illustrates {topic.title.toLowerCase()}. The content structure is ready for a visual asset.</p></section>

    <section className="rounded-3xl border border-gray-200 bg-white p-6 shadow-sm"><h2 className="text-xl font-bold text-gray-900">Exam tips</h2><ul className="mt-4 list-disc space-y-2 pl-5 text-sm leading-6 text-gray-700">{topic.examTips.map((tip) => <li key={tip}>{tip}</li>)}</ul></section>

    <section id="practice" className="rounded-3xl border border-gray-200 bg-white p-6 shadow-sm"><div className="flex flex-col justify-between gap-3 sm:flex-row sm:items-center"><div><h2 className="text-xl font-bold text-gray-900">Practice questions</h2><p className="text-sm text-gray-600">MCQ, theory, programming, practice, and challenge questions are tracked as you study.</p></div><div className="flex gap-2">{["All", "Easy", "Medium", "Hard"].map((level) => <button key={level} type="button" onClick={() => setPracticeDifficulty(level)} className={`rounded-lg px-3 py-2 text-sm font-semibold ${practiceDifficulty === level ? "bg-blue-600 text-white" : "bg-gray-100 text-gray-700"}`}>{level}</button>)}</div></div><div className="mt-5 grid gap-4 lg:grid-cols-3">{topicQuestions.map((question) => <LearningQuestionCard key={question.id} question={question} attempt={attempts[question.id]} onAttempt={recordAttempt} />)}</div></section>

    <section className="rounded-3xl border border-gray-200 bg-white p-6 shadow-sm"><h2 className="text-xl font-bold text-gray-900">Question search & filters</h2><p className="mt-1 text-sm text-gray-600">Search by chapter, topic, keyword, difficulty, or question type.</p><div className="mt-4 grid gap-3 md:grid-cols-5"><label className="md:col-span-2 text-xs font-semibold text-gray-700"><span className="mb-1 block">Search</span><input value={filters.query} onChange={(event) => updateFilter("query", event.target.value)} placeholder="e.g. loop, Java, medium" className="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm" /></label><SelectFilter label="Subject" value={filters.subject} options={options.subjects} onChange={(value) => updateFilter("subject", value)} /><SelectFilter label="Chapter" value={filters.chapter} options={options.chapters} onChange={(value) => updateFilter("chapter", value)} /><SelectFilter label="Difficulty" value={filters.difficulty} options={options.difficulties} onChange={(value) => updateFilter("difficulty", value)} /></div><div className="mt-3 max-w-xs"><SelectFilter label="Question type" value={filters.type} options={options.types} onChange={(value) => updateFilter("type", value)} /></div><p className="mt-4 text-sm font-semibold text-gray-700">{results.length} matching question{results.length === 1 ? "" : "s"}</p><div className="mt-4 grid gap-4 lg:grid-cols-2">{results.slice(0, 6).map((question) => <LearningQuestionCard key={question.id} question={question} attempt={attempts[question.id]} onAttempt={recordAttempt} />)}</div></section>
