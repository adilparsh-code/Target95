...
/**
 * Explanations historically appear as strings, arrays, or objects.
 * Always return an array so renderers can safely use .map().
 */
function normalizeExplanation(explanation) {
    ...
    return explanation
      ...
      .map((item) => {
        ...
        if (typeof item === "object") {
          return item.text || item.description || item.explanation || JSON.stringify(item);
        }
        return String(item);
      })
      .filter(Boolean);
  ...
  if (typeof explanation === "object") {
    const text = explanation.text || explanation.description || explanation.explanation;
    if (text) return Array.isArray(text) ? normalizeExplanation(text) : [String(text)];
...
function normalizeTheory(sd, content) {
  ...
  if (theory) {
    if (theory.beginnerExplanation) sections.push({ type: "paragraph", text: normalizeText(theory.beginnerExplanation) });
    if (Array.isArray(theory.importantPoints) && theory.importantPoints.length) sections.push({ type: "list", title: "Important Points", items: theory.importantPoints });
    if (Array.isArray(theory.memoryTricks) && theory.memoryTricks.length) sections.push({ type: "list", title: "Memory Tricks", items: theory.memoryTricks });
    if (Array.isArray(theory.examTips) && theory.examTips.length) sections.push({ type: "list", title: "Exam Tips", items: theory.examTips });
  }
  ...
  if (Array.isArray(content?.theory)) {
    ...
    sections.push({ type: "paragraph", text: content.theory.trim() });
  }
  ...
  if (content?.introduction?.description) sections.push({ type: "paragraph", text: normalizeText(content.introduction.description) });
  if (sections.length) return sections;

  if (Array.isArray(sd.concepts) && sd.concepts.length) {
    return sd.concepts.map((concept) => ({ type: "paragraph", text: normalizeText(concept) }));
  }
...
function normalizeExample(ex, index, level = null) {
  ...
  return {
    title: ex.title || `${level ? `${level[0].toUpperCase()}${level.slice(1)} ` : ""}Example ${index + 1}`,
    code: ex.code || "",
    output: ex.output !== undefined && ex.output !== null ? ex.output : "",
    explanation: normalizeExplanation(ex.explanation ?? ex.text),
...
function normalizeExamples(sdExamples, contentExamples) {
  ...
  if (Array.isArray(contentExamples)) {
    contentExamples.forEach((ex, index) => {
      ...
      if (normalized) examples.push(normalized);
    });
    ...
    ["basic", "intermediate", "advanced"].forEach((level) => {
      ...
      contentExamples[level].forEach((ex, index) => {
        const normalized = normalizeExample(ex, index, level);
        if (normalized) examples.push(normalized);
      });
    });
  }
...
function normalizePractice(practiceTest) {
  if (!practiceTest || typeof practiceTest !== "object") return null;
  return {
    title: practiceTest.title || "Practice Test",
    totalMarks: practiceTest.totalMarks,
    timeLimit: practiceTest.timeLimit,
    sections: Array.isArray(practiceTest.sections) ? practiceTest.sections : [],
  };
}
...
function normalizeProgramming(questionBankProgramming, contentProgramming) {
  ...
  if (contentProgramming && typeof contentProgramming === "object") {
    ["easy", "medium", "hard"].forEach((level) => {
      ...
      contentProgramming[level].forEach((q) => {
        ...
        programming.push({
          ...
          output: q.output,
          difficulty: level,
          explanation: normalizeExplanation(q.explanation || q.solutionExplanation),
        });
      });
    });
  }
...
function normalizePyqs(previousYearQuestions) {
  if (!Array.isArray(previousYearQuestions)) return null;
...
function normalizeOutput(questionBankOutput, contentOutput, richContentOutput, studyDataOutput) {
  ...
  if (Array.isArray(studyDataOutput)) {
    studyDataOutput.forEach((q, idx) => {
      ...
      output.push({
        id: q?.id || `study-output-${idx}`,
        question: isString ? q : q?.question || q?.prompt || "",
        answer: isString ? "" : q?.answer,
        explanation: isString ? [] : normalizeExplanation(q?.explanation),
        difficulty: isString ? "Medium" : q?.difficulty || "Medium",
        marks: isString ? 2 : q?.marks ?? 2,
      });
    });
  }
...
function normalizeRevisionNotes(contentRevisionNotes, quickRevision) {
  ...
  if (Array.isArray(quickRevision)) {
    quickRevision.forEach((item) => {
      notes.push({ title: "Quick Revision", content: item });
    });
  }
