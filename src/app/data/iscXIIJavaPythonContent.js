...
System.out.println(winner.name); // Priya`,
    practice: [
      ...
      { type: "tracing", question: "Trace: void swap(Student a, Student b) { Student t = a; a = b; b = t; } called as swap(s1, s2). Do s1 and s2 in main swap?", answer: "No. Only the copies of the references inside swap are exchanged. The caller's s1 and s2 still refer to the original objects." },
      { type: "debugging", question: "A static method tries to print this.marks. What error occurs and why?", answer: "Compile-time error: a static method has no receiving object, so this is not available. Use an instance method or pass a Student parameter." },
      { type: "programming", question: "Write a method Student merge(Student a, Student b) that returns a new Student whose marks are the sum of a and b and whose name is a.name + \" & \" + b.name.", answer: "return new Student(a.name + \" & \" + b.name, a.marks + b.marks); The original objects are left unchanged." },
      { type: "mcq", question: "Can two overloaded methods differ only in return type?", options: ["A) Yes", "B) No", "C) Only if they are static", "D) Only if they are void"], answer: "B", explanation: "The parameter list must differ; return type alone cannot overload." },
    ],
    related: [
      { href: "/isc/class-xii/inheritance-interface", label: "Inheritance and Interface" },
...
// Returns: 1 -> 2 -> 6 -> 24`,
    practice: [
      ...
      { type: "tracing", question: "Trace fib(4) for fib(n)=fib(n-1)+fib(n-2), fib(0)=0, fib(1)=1. How many base-case returns occur?", answer: "fib(4)→fib(3)+fib(2); fib(3)→fib(2)+fib(1); fib(2)→fib(1)+fib(0). Base-case returns: fib(1), fib(0), fib(1), fib(1), fib(0) — five. Value 3." },
      { type: "debugging", question: "gcd(a,b) is written as return gcd(a, b) with no other change. Identify the defect.", answer: "The arguments never reduce, so the base case is never reached. Use gcd(b, a % b) with base case b==0 returning a." },
      { type: "programming", question: "Write a recursive method int gcd(int a, int b) using Euclid's algorithm.", answer: "if (b == 0) return a; return gcd(b, a % b);" },
      { type: "mcq", question: "Recursive binary search extra stack space is typically", options: ["A) O(1)", "B) O(log n)", "C) O(n)", "D) O(n²)"], answer: "B", explanation: "Each call halves the range, so depth is O(log n)." },
    ],
    related: [
      { href: "/isc/class-xii/trees", label: "Trees" },
      { href: "/isc/class-xii/complexity-big-o", label: "Complexity and Big O" },
...
// a.fetch(); // compile error: fetch() not in Animal`,
    practice: [
      ...
      { type: "tracing", question: "class A { int x=1; } class B extends A { int x=2; void show(){ System.out.println(x+\" \"+super.x);} } new B().show(); State the output.", answer: "2 1. x in B shadows A's x; super.x reads the superclass field." },
      { type: "debugging", question: "Subclass constructor does not call super(n) but the superclass has only a parameterized constructor. What happens?", answer: "Compile-time error. Java inserts super() (no-arg) automatically; that constructor does not exist. Write super(n) as the first statement." },
      { type: "programming", question: "Write interface Drawable { void draw(); } implemented by Circle and Rectangle. Store both in a Drawable[] and call draw() in a loop.", answer: "Drawable[] items = { new Circle(), new Rectangle() }; for (Drawable d : items) d.draw(); Each object's draw() runs by dynamic dispatch." },
      { type: "mcq", question: "super() in a subclass constructor must appear", options: ["A) Anywhere in the constructor", "B) As the first statement", "C) After field initialization", "D) Only in main"], answer: "B", explanation: "The superclass constructor must complete before subclass initialization." },
    ],
    related: [
      { href: "/isc/class-xii/methods-object-parameter-return", label: "Methods – objects as parameters" },
      { href: "/isc/class-xii/recursion", label: "Recursion" },
    ],
  },
...
def word_frequency(words):
    ...
    practice: [
      ...
      { type: "tracing", question: "Trace:\ndef f(x, y=2):\n    return x * y\nprint(f(3))\nprint(f(3, 4))", answer: "First call uses default y=2 → 6. Second call uses y=4 → 12. Output:\n6\n12" },
      { type: "debugging", question: "d = {'a': 1}; print(d['b']) raises KeyError. How should a missing key be handled in ISC-style code?", answer: "Use d.get('b', 0) (or another default) instead of d['b'], or test 'b' in d before indexing." },
      { type: "programming", question: "Write a function that takes a list of marks and returns a dict with keys 'max', 'min' and 'avg'.", answer: "return {'max': max(marks), 'min': min(marks), 'avg': sum(marks)/len(marks)} after handling an empty list." },
      { type: "mcq", question: "What does a Python function return if it has no return statement?", options: ["A) 0", "B) False", "C) None", "D) It will not compile"], answer: "C", explanation: "The implicit return value is None." },
    ],
    related: [
      { href: "/isc/class-xii/arrays-strings", label: "Arrays and Strings (Java)" },
      { href: "/isc/class-xii/complexity-big-o", label: "Complexity and Big O" },
    ],
  },
