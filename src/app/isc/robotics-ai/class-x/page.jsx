...
export default function ICSEAIClassXPage() {
  const questionCount = ai.units.reduce((total, unit) => total + unit.questions.length, 0);
  const totalMarks = ai.units.reduce((total, unit) => total + unit.weightage, 0);
      ...
      <div className="mx-auto max-w-6xl px-6 py-10">
        ...
        <header className="mt-6 rounded-[2rem] bg-gradient-to-br from-violet-950 via-fuchsia-900 to-slate-950 p-8 text-white md:p-10">
          ...
          <div className="mt-6 flex flex-wrap gap-3 text-sm">
            <span className="rounded-full bg-white/10 px-3 py-1.5">{ai.units.length} AI units</span>
            <span className="rounded-full bg-white/10 px-3 py-1.5">{totalMarks} marks represented</span>
            <span className="rounded-full bg-white/10 px-3 py-1.5">{questionCount} board-style questions</span>
          </div>
        </header>
            ...
            <section id={unit.id} key={unit.id} className="scroll-mt-6 rounded-3xl border border-slate-200 bg-white p-6 shadow-sm md:p-8">
              ...
              <div className="mt-6 grid gap-6 lg:grid-cols-2">
                <div>
                  ...
                  <ul className="mt-3 space-y-2 pl-5 text-sm text-slate-600">{unit.topics.map((topic) => <li key={topic} className="list-disc">{topic}</li>)}</ul>
                </div>
                <div>
                  <h3 className="text-lg font-bold">Core theory</h3>
                  <ul className="mt-3 space-y-2 pl-5 text-sm leading-6 text-slate-600">{unit.theory.map((point) => <li key={point} className="list-disc">{point}</li>)}</ul>
                </div>
              </div>

              {unit.workedExamples?.length ? <div className="mt-6 grid gap-4 md:grid-cols-2">{unit.workedExamples.map((example) => <div key={example.title} className="rounded-2xl bg-slate-50 p-5"><h3 className="font-bold">{example.title}</h3><pre className="mt-3 overflow-x-auto whitespace-pre-wrap rounded-xl bg-slate-950 p-4 text-xs text-slate-100">{example.code}</pre><p className="mt-3 text-sm text-slate-600">{example.conclusion}</p></div>)}</div> : null}

              <div className="mt-7"><h3 className="text-xl font-bold">Board-style practice</h3><div className="mt-4 grid gap-3">{unit.questions.map((question) => <Question key={question.id} question={question} />)}</div></div>
            </section>
          ))}
        </div>
        ...
        <section className="mt-8 rounded-3xl border border-violet-200 bg-violet-50 p-6 md:p-8">
          ...
          <ul className="mt-4 grid gap-3 md:grid-cols-2">{ai.practicalAssignments.map((item) => <li key={item} className="rounded-2xl bg-white p-4 text-sm text-violet-950">{item}</li>)}</ul>
        </section>
      </div>
    </main>
  );
