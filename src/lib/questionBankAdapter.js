...
const QUESTION_TYPES = ["mcq","programming","output","output-tracing","theory","fill-blank","true-false","debugging","case-based"];
...
export function normalizeQuestion(question) {
  ...
  const board = question.board || meta.board || (String(question.id).startsWith("icse-") ? "ICSE" : undefined);
  const className = question.class || meta.class || (String(question.id).startsWith("icse-") ? "X" : undefined);
  const chapter = question.chapter || "";
  const chapterSlug = question.chapterSlug || question.slug || slugify(chapter);

  return {
    id: String(question.id), subject: question.subject || meta.subject || "Computer Science",
    board, class: className, syllabusUnit: question.syllabusUnit || meta.syllabusUnit,
    chapter, chapterSlug, topic: question.topic || meta.topic || "General",
    difficulty: question.difficulty || "Medium",
    questionType: QUESTION_TYPES.includes(questionType) ? questionType : "theory",
    marks: question.marks ?? 1, question: question.question || question.prompt || "",
    options: Array.isArray(question.options) ? question.options : [],
    correctAnswer: question.correctAnswer ?? question.answer ?? question.modelAnswer ?? question.javaSolution ?? "",
    explanation: question.explanation || "", codeSnippet: question.codeSnippet || question.javaSolution || "",
...
export const questionBankQuestions = [...allJavaChapterQuestions, ...ICSE_BOARD_PRACTICE_2026_27].map(normalizeQuestion);
...
export function filterQuestionBank(questions, filters) {
  ...
  return questions.filter((question) => {
    ...
    return (!term || searchable.includes(term)) &&
      ...
      (filters.questionType === "all" || question.questionType === filters.questionType) && matchesStatus;
  });
}

export function getQuestionBankChapter(slug) {
  return questionBankQuestions.filter((q) => q.chapterSlug === slug);
}
